    ...
    for category in os.listdir(dataset_dir):
      ...
      for vid_name in tqdm(os.listdir(folder_path),total=len(os.listdir(folder_path))):
          ...
          for scene_id in tqdm(range(1,len(os.listdir(vid_path))-1),total=len(os.listdir(vid_path))-1):
            ...
            for state in states:
                if category == 'flour':
                    question = "You'll see a small segment of an entire video with a topic of '"+vid_topic+"', whose content is about'"+vid_des+"' Based on the given segment, is it plausible for the "+category+" to be "+state+" after this operation in this video? Directly output 'yes' or 'no' without other statements."
                else:
                    question = "This is a small segment of an entire video with a topic of '"+vid_topic+"'. Based on the possible following step of this video segment, is it plausible for the "+category+" to be "+state+" after this step? Directly output 'yes' or 'no' without other statements." 
                ...
                if 'yes' in output_lower:
                    ...
                    state_prob = 0.5
                    state_probs[state] = str(state_prob.item())
            save_dict[category][vid_name][clip]={'caption':pred_states,'state_prob':state_probs,'segment':clip_dict[scene_id]}
          json.dump(save_dict,open('data/forward_temporal_dependency_scores.json','w'))
